...
LEGISTAR_API_BASE = "https://webapi.legistar.com/v1/richmondva"
...
class RichmondCitySource(CivicSource):
    """richmond city council - meetings from legistar calendar, legislation from legistar web api."""
    ...
    def __init__(self):
        ...
        self._recent_legislation = []  # cache of legislation from the web api
    ...
    async def fetch_updates(self) -> str:
        ...
        try:
            ...
            # hint that legislation tracking is available
    ...
    def _get_legislation_summary(self) -> str:
        ...
        return "**Pending Legislation:** Say 'richmond legislation' for active ordinances and resolutions"
    ...
    async def get_details(self, item_ref: str) -> str:
        ...
        if leg_match:
            ...
            return (
                ...
                f"Say 'richmond legislation' to hear all recent items."
            )

        # try legislation text search when the query looks like a topic/description
        ...
        # fetch agenda items from the web api (the scraped meeting id is the api EventId)
        agenda_items = []
        if meeting.get('id'):
            try:
        ...
        if agenda_items:
            # prefer items tied to legislation (matter file) over procedural boilerplate
