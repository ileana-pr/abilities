...
class RichmondCitySource(CivicSource):
    ...
    async def fetch_updates(self) -> str:
        ...
        try:
            ...
            meeting_matches = re.findall(r'MeetingDetail\.aspx\?ID=(\d+)&amp;GUID=([0-9A-F-]+)', html)
            agenda_matches = re.findall(r'View\.ashx\?M=A&amp;ID=(\d+)&amp;GUID=([0-9A-F-]+)', html)
            
            updates = ["### Richmond City Council (Upcoming)"]
            for i in range(min(5, len(meeting_matches))):
                agenda_status = "✅ Agenda Available" if i < len(agenda_matches) else "❌ No Agenda Yet"
                updates.append(f"- **Meeting ID**: {meeting_matches[i][0]} | {agenda_status}")
            
            return "\n".join(updates) if len(updates) > 1 else "No upcoming Richmond meetings found."
        except Exception as e:
            return f"Error fetching Richmond data: {str(e)}"
